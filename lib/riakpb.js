...
//     repeated bytes keys = 1;
// }
var decode_IndexQueryResp = function(stream, res) {
    res = res || {keys:[]};
    decodeLoop(stream, function(type, fieldnum) {
        if (fieldnum == 1 && type == 2) {
            res.keys.push(readString(stream));
...
var addParameters = function(buf, idx, params, repeat) {
    var n = params.length;
    for (var i = 0; i < n; i++) {
        var p = params[i];
        var storeindex = repeat? repeat : (i + 1);
        var type = typeof p;
        if (type == "string") {
            buf[idx] = (storeindex << 3) + 2;
            ...
            // Note: this is used for array objects, not dictionaries.
            // Arrays can have the extra property riakpb_repeat for arrays of objects
            if (!p.riakpb_repeat)
                buf[idx] = (storeindex << 3) + 2;
            ...
            do {
                tempv = new Buffer(l);
                l = addParameters(tempv, 0, p, p.riakpb_repeat? storeindex : undefined);
...
// }
var encode_IndexQueryReq = function(bucket, index, min, max) {
    if (max === undefined || min === max)
        return makeMessage(25, [bucket, index, 0, min]);
    return makeMessage(25, [bucket, index, 1, undefined, min, max]);
}
    ...
    if (this.state == 'disconnected') {
        ...
        var handler = function() {
            ...
            if (self.connectCallback) {
                self.connectCallback(null, self);
            }
            self.nextRequest();
        };
        this.client = net.connect(this.port, this.host, handler);
        this.client.on('error', function(data) {
            self.state = 'disconnected';
            if (self.connectCallback) {
                self.connectCallback(err, null);
            }
        });
    }
}
...
Client.prototype.end = function() {
    if (this.state != 'disconnected') {
        this.client.end();
        // Let the end handler deal with teardown
    }
}
...
Client.prototype.processPacket = function(stream, msg, len) {
    ...
    if (msg == 0) {
        ...
        this.res = decode_IndexQueryResp(stream, this.res);
    } else if (msg == 28) {
        this.res = decode_SearchQueryResp(stream, this.res);
    } else {
        stream.idx += len;
        this.res = {};
    }
...
Client.prototype.PutIndex = function(bucket, key, data, indexes, options, cb) {
    options = options || {};
    options.content = options.content || {};
    options.content.indexes = indexes;
    indexes.riakpb_repeat = true;
    this.send(encode_PutReq(bucket, key, data, options), cb);
}

Client.prototype.PutIndexNokey = function(bucket, data, indexes, options, cb) {
    options = options || {};
    options.content = options.content || {};
    options.content.indexes = indexes;
    indexes.riakpb_repeat = true;
...
Client.prototype.IndexQuery = function(bucket, index, value, cb) {
    if (typeof value == "number") value = "" + value;
    this.send(encode_IndexQueryReq(bucket, index, value), cb);
}

Client.prototype.IndexQueryRange = function(bucket, index, min, max, cb) {
    if (typeof min == "number") min = "" + min;
    if (typeof max == "number") max = "" + max;
    this.send(encode_IndexQueryReq(bucket, index, min, max), cb);
}
